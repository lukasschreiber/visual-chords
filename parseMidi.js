import fs from "fs";
const midiData = fs.readFileSync('./frontend/src/components/song/midi/Oh_Come_All_Ye_Faithful.mid', 'hex');
const bytes = midiData.match(/\S\S/g);

const htob = (hex) => Number.parseInt(hex, 16).toString(2).padStart(8, "0");
...
const readHeader = (bytes) => {
    ...
    const divisionRaw = bytes.slice(4, 6).map(hex => htob(hex)).join("");
...
fs.writeFileSync("./midi.json", JSON.stringify(jsonOut));